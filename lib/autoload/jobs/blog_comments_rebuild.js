...
module.exports = function (N) {
  N.wire.on('init:jobs', function register_blog_comments_rebuild() {
    // Iterator
    //
    N.queue.registerTask({
      name: 'blog_comments_rebuild',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'blog_comments_rebuild',
      ...
      async iterate(state) {
        ...
        let chunks = _.chunk(posts.map(p => String(p._id)), POSTS_PER_CHUNK)
                      .map(ids => N.queue.blog_comments_rebuild_chunk(ids));
      ...
      async init() {
        let query = N.models.blogs.BlogComment.count();

        if (this.args.length < 1 || !this.args[0]) {
          // if no min _id
          let min_post = await N.models.blogs.BlogComment.findOne()
                                   .select('_id')
                                   .sort('_id')
                                   .lean(true);
        ...
        if (this.args.length < 2 || !this.args[1]) {
          // if no max _id
          let max_post = await N.models.blogs.BlogComment.findOne()
                                   .select('_id')
                                   .sort('-_id')
                                   .lean(true);
    ...
    //
    N.queue.registerTask({
      name: 'blog_comments_rebuild_chunk',
      pool: 'hard',
      removeDelay: 3600,
      async process(ids) {
        let start_time = Date.now();

        N.logger.info(`Rebuilding blog comments ${ids[0]}-${ids[ids.length - 1]} - ${ids.length} found`);

        await Promise.map(
          ids,
          id => N.wire.emit('internal:blogs.blog_comment_rebuild', id)
                      .catch(err => N.logger.error(`Failed to rebuild blog comment id=${id}: ${err.stack || err}`))
        );

        N.logger.info(`Rebuilding blog comments ${ids[0]}-${ids[ids.length - 1]} - finished (${
