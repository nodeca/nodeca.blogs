    ...
    N.queue.registerTask({
      ...
      async process() {
        try {
          ...
          try {
            await N.redis.rename('views:blog_entry:count', 'views:blog_entry:count_tmp');
          } catch (__) {}

          let items = await N.redis.hgetall('views:blog_entry:count_tmp');

          await N.redis.del('views:blog_entry:count_tmp');
          ...
          await N.redis.zremrangebyscore('views:blog_entry:track_last', '-inf', score);
        } catch (err) {
