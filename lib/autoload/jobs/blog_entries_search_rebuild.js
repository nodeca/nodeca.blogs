// Reindex all blog entries (iterator started from admin interface)
//
'use strict';
...
module.exports = function (N) {
  ...
  N.wire.on('init:jobs', function register_blog_entries_search_rebuild() {

    N.queue.registerTask({
      name: 'blog_entries_search_rebuild',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'blog_entries_search_rebuild',
      ...
      async iterate(state) {
        ...
        let chunks = _.chunk(posts.map(p => String(p._id)), POSTS_PER_CHUNK)
                      .map(ids => N.queue.blog_entries_search_update_by_ids(ids, { shadow: true }));
      ...
      async init() {
        let query = N.models.blogs.BlogEntry.countDocuments();
