// Keeping track of a number of blog comments made by each user
//
...
module.exports = function (N, collectionName) {
  ...
  let UserBlogCommentCount = new Schema({
  ...
  UserBlogCommentCount.index({ user: 1 });
   ...
   */
  UserBlogCommentCount.statics.inc = async function inc(user_id, { is_hb }) {
    let data = await N.models.blogs.UserBlogCommentCount.findOneAndUpdate(
      { user: user_id },
      {
        $inc: {
          value: is_hb ? 0 : 1,
   ...
   */
  UserBlogCommentCount.statics.recount = async function recount(user_id) {
    let bulk_data;
  ...
  N.wire.on('init:models', function emit_init_UserBlogCommentCount() {
    return N.wire.emit('init:models.' + collectionName, UserBlogCommentCount);
  });


  N.wire.on('init:models.' + collectionName, function init_model_UserBlogCommentCount(schema) {
