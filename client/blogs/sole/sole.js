...
N.wire.on('navigate.done:' + module.apiPath, function page_setup(data) {
  ...
  pageState.reached_end        = last_entry_hid === $('.blogs-sole__entry-list > :last').data('entry-hid');
  pageState.prev_loading_start = 0;
  pageState.next_loading_start = 0;
  pageState.top_marker         = $('.blogs-sole').data('top-marker');
  pageState.bottom_marker      = $('.blogs-sole').data('bottom-marker');
  ...
  if (pagination.chunk_offset > 1 && $('.blogs-sole__entry-list').length) {
    $window.scrollTop($('.blogs-sole__entry-list').offset().top - $('.navbar').height());
...
function reset_loading_placeholders() {
  let prev = $('.blogs-sole__loading-prev');
  let next = $('.blogs-sole__loading-next');
...
N.wire.once('navigate.done:' + module.apiPath, function blogs_sole_init_handlers() {
  ...
  N.wire.on(module.apiPath + ':load_prev', function load_prev_page() {
    ...
    N.io.rpc('blogs.sole.list.by_range', {
      ...
      let old_height = $('.blogs-sole__entry-list').height();
      ...
      return N.wire.emit('navigate.update', {
        $:       $result,
        locals:  res,
        $before: $('.blogs-sole__entry-list > :first')
      }).then(() => {
        // update scroll so it would point at the same spot as before
        $window.scrollTop($window.scrollTop() + $('.blogs-sole__entry-list').height() - old_height);
  ...
  N.wire.on(module.apiPath + ':load_next', function load_next_page() {
    ...
    N.io.rpc('blogs.sole.list.by_range', {
      ...
      return N.wire.emit('navigate.update', {
        $:      $result,
        locals: res,
        $after: $('.blogs-sole__entry-list > :last')
      }).then(() => {
        //
        // Limit total amount of posts in DOM
        //
        ...
        if (cut_count > CUT_ITEMS_MAX - CUT_ITEMS_MIN) {
          ...
          if (entry.getBoundingClientRect().bottom < -600) {
            ...
            // update scroll so it would point at the same spot as before
            $window.scrollTop(old_scroll + $('.blogs-sole__entry-list').height() - old_height);
