...
N.wire.once(module.apiPath, function entry_list_mixed_setup_handlers() {
  ...
  N.wire.on(module.apiPath + ':entry_expand', async function expand(data) {
    ...
    const res = await N.io.rpc('blogs.index.list.by_ids', { entry_ids: [ entryId ] });

    let $result = $(N.runtime.render('blogs.blocks.entry_list_mixed', Object.assign(res, { expand: true })));
  ...
  N.wire.on(module.apiPath + ':add_infraction', async function add_infraction(data) {
    ...
    const res = await N.io.rpc('blogs.index.list.by_ids', { entry_ids: [ entry_id ] });
